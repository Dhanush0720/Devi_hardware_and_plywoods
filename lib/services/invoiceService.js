...
async function saveBuffer(invoiceNumber, buffer) {
  ...
  try {
    const pdfUrl = await uploadBuffer(buffer, filename, 'invoices');
    const isCloud = process.env.BLOB_READ_WRITE_TOKEN || process.env.CLOUDINARY_CLOUD_NAME;
    const filePath = isCloud ? null : path.join(process.cwd(), 'public', 'invoices', filename);
    return { pdfUrl, filePath };
  } catch (err) {
    console.error('[invoiceService] saveBuffer failed:', err);
    throw err;
  }
}
